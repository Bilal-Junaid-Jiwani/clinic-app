...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen bg-[#edf2f7] font-sans selection:bg-[#14b8a6] selection:text-white">
            <SidebarClient role={role} name={name || "User"} links={getLinks()} />

            <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#14b8a6]/[0.025] rounded-full blur-3xl pointer-events-none" />

                <header className="bg-white/80 backdrop-blur-xl px-6 lg:px-10 py-4 flex items-center justify-between border-b border-gray-200/60 z-10 sticky top-0">
                    <div className="pl-12 lg:pl-0">
                        <h2 className="text-xl font-black text-gray-900 tracking-tight">Workspace</h2>
                        <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">
                            {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                        </p>
                    </div>
                    <div className="flex items-center gap-3 bg-white border border-gray-100 px-4 py-2 rounded-full shadow-sm hover:shadow-md transition-shadow">
                        <div className="relative flex h-2.5 w-2.5">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#14b8a6] opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#0d9488]"></span>
                        </div>
                        <span className="text-[11px] text-gray-600 font-extrabold tracking-wider uppercase">Online</span>
                    </div>
                </header>
