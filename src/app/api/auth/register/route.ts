...
export async function POST(req: Request) {
    try {
        ...
        // Send 'Payment Processing' Receipt instantly (don't block registration if email fails)
        try {
            await sendEmail({
                to: email,
                subject: "Aether Architecture Provisioning - Payment Processing",
                html: `
                <div style="font-family: sans-serif; max-w: 600px; margin: 0 auto; background: #0F0B2E; color: white; padding: 40px; border-radius: 16px;">
                    ...
                    <p style="color: #A9A3C2; font-size: 16px; line-height: 1.6; text-align: center; margin-bottom: 32px;">
                        ...
                        <br/><br/>
                        Your node is currently undergoing Aether Governance provisioning. You will receive an automated confirmation email containing your final access keys once the deployment activates.
                    </p>
            ...
            console.error("Email delivery protocol failure:", emailError);
            // We continue as the user's account is already saved in the DB
        }
